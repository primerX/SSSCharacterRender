...
float3 DiffuseLightingFull(float3 WorldPos, float3 Normal, half Wrap, float3 WorldView, 
float2 lightmapUV, float3 GI, float LightClamp)
{
    ...
    float3 Lambert = OrenNayar(Color * DistanceAtten, Direction, Normal, Wrap, WorldView);
    ...
    float3 AdditionalsDiffuse = diffuseColor;
    
    return MainLightDiffuse + AdditionalsDiffuse;

}

float3 SpecularLightingFull(float3 WorldPos, float3 Normal, float3 SpecColor, float Smoothness, float3 WorldView, float2 lightmapUV)
{
#if defined(_MAIN_LIGHT_SHADOWS_SCREEN) && !defined(_SURFACE_TYPE_TRANSPARENT)
		float4 shadowCoord = ComputeScreenPos(TransformWorldToHClip(WorldPos));
#else
    ...
    float4 Shadowmask = SAMPLE_SHADOWMASK(lightmapUV);
    float ShadowAtten = MainLightShadow(shadowCoord, WorldPos, Shadowmask, _MainLightOcclusionProbes);
    float3 Lambert = LightingLambert(Color * DistanceAtten, Direction, Normal);
    float3 GI = 0;
    MixRealtimeAndBakedGI(DirectionalLight, Normal, GI); //TODO
    //float3 MainLightDiffuse = /* Cookie **/ShadowAtten * Lambert /* + GI*/;
    Smoothness = exp2(10 * Smoothness + 1);

    float3 attenuatedLightColor = Color * (DistanceAtten * ShadowAtten);
    float3 specularColor = LightingSpecular(attenuatedLightColor, Direction, Normal, WorldView, float4(SpecColor, 0), Smoothness);
