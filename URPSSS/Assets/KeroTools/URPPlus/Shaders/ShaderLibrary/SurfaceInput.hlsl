...
half3 SampleBentNormal(float2 uv, TEXTURE2D_PARAM(bentNormalMap, sampler_bentNormalMap), half scale = half(1.0))
{
...
half3 SampleCoatNormal(float2 uv, TEXTURE2D_PARAM(coatNormalMap, sampler_coatNormalMap), half scale = half(1.0))
{
    #if defined(_COATNORMALMAP)
    half4 normal = SAMPLE_TEXTURE2D(coatNormalMap, sampler_coatNormalMap, uv);
    return ScaleNormal(normal, scale);
    #else
    return half3(0.0h, 0.0h, 1.0h);
    #endif
