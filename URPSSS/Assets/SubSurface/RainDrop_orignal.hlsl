...
half4 RainDropFunction(Varyings i)
{
    ...
    float2 uv = ((i.uv * _ScreenParams.xy) - .5*_ScreenParams.xy) / _ScreenParams.x;
    float2 UV = i.uv.xy;
    ...
    // half4 waterMask =  SAMPLE_TEXTURE2D(_WaterMask, sampler_WaterMask, i.uv);
    // n *= waterMask.r;
    ...
    half4 lod =  SAMPLE_TEXTURE2D_LOD(_BaseMap, sampler_BaseMap, texCoord, focus);
...
float2 WaterLayer(float2 uv, float t)
{
    float2 UV = uv;
...
float2 Drops(float2 uv, float t, float layer2) 
{
    float2 m2 = WaterLayer(uv * _DynamiceLayer2Tiling, t * _DynamicRainDropSpeed) * layer2;
...
float2 DropLayerDebug(float2 uv, float t)
{
    uv.y += t;
...
half4 frag(Varyings i)
{
    ...
    float2 c = Drops(uv, t, layer2);

    float2 e = float2(0.001, 0.0);
    float cx = Drops(uv + e, t, layer2).x;
    float cy = Drops(uv + e.yx, t, layer2).x;
    float2 n = float2(cx - c.x, cy - c.x);
